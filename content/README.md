...
| `stories.csv` | 꽃에 얽힌 일화(창작 외 출처 필수) | 449 |
...
`templates.csv` 의 62행은 **(마음 8종 × 화면에 서는 톤) 31칸 × 길이 2벌**이다(사과는 유쾌
톤을 화면에서 내리므로 3칸 — 그 자리에 행을 만들면 어디에도 안 나가는 죽은 데이터가 된다).
한 칸이라도 비면 그 조합에서 길이 토글이 조용히 아무 일도 하지 않으므로, 격자는
`tests/seed/schemas.test.ts` 가 지킨다. 분량은 **짧게 80~140자(두세 문장) · 보통 220~320자
(네다섯 문장)** 이고 이 띠는 LLM 생성 경로가 요구받는 띠와 같은 값이다(`design-spec.md`
§1.5s ③) — 키가 없는 배포에서 예문이 그대로 화면에 서기 때문에 두 경로의 분량이 어긋나면
안 된다. **문안 규칙은 LLM 프롬프트의 금지선과 같다**: 꽃 이름·가격·배송·재고를 쓰지 않고
(3안 어디에 붙어도 성립해야 한다), 반려동물 안전·알레르기를 말하지 않으며, 이름 자리표시자
(`{이름}`·`OO`)를 쓰지 않고, 관계 칸은 비워 관계 중립으로 둔다(값을 채운 행은 사과 2행뿐이다).
사과 행은 `required_apology_elements` 에 담아야 할 요소(인정·책임·재발 방지·약속)를 적어 둔다.
2026-08-18 개편으로 기존 30행이 `medium` → `short` 로 **재라벨**됐고(문안 불변) `medium` 31행이
새로 들어왔다. `tpl-apology-plain` 만 문안을 다시 썼다 — 31자 두 문장이라 새 `짧게` 기준에
닿지 못했다.
...
- **실자료 조사로 들어온 행은 조사 회차별 접두사를 쓴다.** 접두사로 "검토 전 샘플"과 "출처를 직접
  ...
  - `weekly-research(2026-08-17):` — 두 번째 주간 콘텐츠 리서치. 신규 꽃 대신 **한국 유통
    ...
    7편을 조사해 붙였다(조사 착수 시점 기준 `stories.csv` 377 → 384). 이 세 종은 그
    전까지 이야기가 0편이었다(`meanings.csv` 는 이번 회차에서 늘지 않았다). 이 회차와
    별개로 같은 날 카탈로그 확장 배치 2(`docs/flowers-batch2-research.md`, 신규 12종 +
    `stories.csv` 61편)가 먼저 `main` 에 들어가 있었고, 두 작업이 독립적으로 갈라져
    나갔다가 병합됐다. 병합 뒤 진행한 두 번째 검증(2026-08-18, `main` 이 story_id
    유일성 검사를 새로 추가한 배치와 재병합하며 드러남)에서 **`story-chrysanthemum-double-ninth`
    가 seed-v4(`docs/story-research-2.md` #45, 2026-08-15)와 소재·id가 완전히 겹치는
    것을 확인해 이번 회차 쪽 행을 뺐다** — 두 행 모두 비장방·환경의 중양절 국화주 설화를
    다뤄, "같은 소재는 두 번 싣지 않는다" 원칙(§`stories.csv` 절 참고)에 따른 조치다.
    그래서 이 회차의 실제 기여는 6편이고, `stories.csv` 최종 행 수는 438 → **444** 다.
    조사 경위·소스 유형 분포·제외 목록의 단일 원본은 `docs/weekly-research-2026-08-17.md`
    다(이 문서의 수치는 조사 착수 시점 기준이며, 병합·중복 제거 후 전체 분포는 아래
    표를 따른다). **이 세션은 `WebFetch` 가 조직 egress 정책으로 차단돼**(2026-08-15
    회차와 같은 제약) `WebSearch` 스니펫 교차 확인으로 대체했다 — 문서 §1 참고.
  ...
  - `weekly-research(2026-08-31):` — 세 번째 주간 콘텐츠 리서치. 다시 트랙 B(한국 유통
    상위 꽃 심화)를 택해, 2회차가 다루지 못한 나머지 후보 중 이야기 보유량이 가장 적던
