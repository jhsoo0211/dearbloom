// deck-d1 (모바일 쇼케이스) 미리보기 촬영 — 정적 서버 3421 전용 + Playwright 1920x1080
//
// 포트 오염 사고 전례가 있어 이 덱은 3421 한 곳에만 바인딩한다.
// 3421 이 이미 쓰이고 있으면 다른 포트로 흘러가지 않고 즉시 실패한다.
// 촬영 직전 <meta name="deck" content="d1"> 마커를 두 번 검증한다(서빙 / 렌더).
...
const PORT = 3421;
const MARKER = 'd1';
const MARKER_TAG = '<meta name="deck" content="d1">';
...
// 루프백 3421 전용. 점유돼 있으면 다른 포트로 흘러가지 않고 그대로 실패한다.
await new Promise((resolve, reject) => {
  server.once('error', (e) => reject(new Error(`포트 ${PORT} 바인딩 실패: ${e.code}. 다른 포트로 우회하지 않는다.`)));
  server.listen(PORT, '127.0.0.1', resolve);
});
console.log('serving deck-d1 on http://127.0.0.1:' + PORT);

// 내가 띄운 서버가 정말 deck-d1 을 서빙하는지 확인 (다른 덱 서버로 새는 것 방지)
{
  const probe = await fetch(`http://127.0.0.1:${PORT}/slide1.html`).then((r) => r.text());
  if (!probe.includes(MARKER_TAG)) {
    throw new Error(`포트 ${PORT} 가 deck-d1 을 서빙하지 않습니다. 다른 서버와 충돌.`);
  }
  console.log('서빙 검증 OK — deck-d1 마커 확인');
}
