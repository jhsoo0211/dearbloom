// deck-d2 미리보기 촬영 — 정적 서버(3422 전용) + Playwright 1920x1080
// preview-N.png (검수용) 와 final-N.jpg (제출용) 를 함께 만든다.
...
// 이 변주는 3422 만 쓴다. 3000·3400 은 다른 작업자/데모 몫이라 건드리지 않는다.
const PORT = 3422;
const MARKER = 'd2';
...
// 루프백에만 바인딩. 3422 가 막혀 있으면 다른 포트로 새지 않고 그냥 멈춘다.
await new Promise((resolve, reject) => {
  server.once('error', (e) => reject(new Error(`포트 ${PORT} 를 열지 못했습니다: ${e.code}`)));
  server.listen(PORT, '127.0.0.1', resolve);
});
console.log('serving deck-d2 on http://127.0.0.1:' + PORT);

// 내 파일이 맞는지 확인 (다른 작업자 서버로 새는 것 방지)
{
  const probe = await fetch(`http://127.0.0.1:${PORT}/slide1.html`).then((r) => r.text());
  if (!probe.includes(`content="${MARKER}"`)) {
    throw new Error(`포트 ${PORT} 가 deck-d2 를 서빙하지 않습니다. 다른 서버와 충돌.`);
  }
  console.log('서빙 검증 OK — deck-d2 마커 확인');
}
...
try {
  ...
  for (const i of pages) {
    ...
    await page.screenshot({ path: path.join(DIR, `preview-${i}.png`), clip });
    await page.screenshot({ path: path.join(DIR, `final-${i}.jpg`), clip, type: 'jpeg', quality: 94 });
