...
/**
 * 고쳐 쓰기 상한 — 편집 전용 상수(`MESSAGE_MAX_CHARS`)를 그대로 쓴다.
 *
 * ⚠ 이 값은 **생성 상한이 아니다.** 생성은 길이 축마다 따로다(`MESSAGE_LENGTH_MAX_CHARS`).
 *   360 인 이유는 `보통`(340) 을 통째로 붙여 넣고도 손볼 자리가 남아야 하기 때문이다 —
 *   같은 수를 두면 우리가 내준 문장이 이미 칸을 꽉 채운 채로 열려서, 고쳐 쓰기가
 ...
 * 있으면 상한을 옮긴 날 화면만 옛말을 하게 된다 — 사용자에게 140자라고 해 놓고 200자를
 * 받아 주는 자리가 생기지 않게, 말하는 수와 막는 수를 한 벌로 묶는다.
 *
 * ⚠ 화면은 **문장 수를 말하지 않는다**(`MESSAGE_LENGTH_SENTENCES` 는 프롬프트 전용이다).
 *   말하는 수는 우리가 막을 수 있는 수여야 하는데 문장 수는 세지 않기로 한 값이고
 *   (한국어 문장 경계는 기계로 못 가른다), 예문 원장에도 짧은 문장 넷으로 된 `짧게` 가
 *   있어서 "두세 문장"이라고 적으면 그 자리에서 곧바로 어긋난다.
 */
...
export default function ResultView({
   ...
   * 길이·새로 받기를 세울 수 있는가.
   *
   * 두 조건이 함께 맞아야 한다: 상위가 재생성 길을 줬고(복원된 결과에는 없다),
   * 지금 이 톤이 **생성된 문장**이거나 상위가 `canReword` 로 갈아 볼 문장이 있다고 말했다.
   *
   * 흘러들어오는 중(`writing`)에는 서지 않는다 — 아직 확정되지 않은 문장을 두고
   * "새로 받기" 를 누를 수 있게 하면 두 요청이 겹친다.
   *
   * ⚠ **본배포의 예문 톤에서는 여전히 서지 않는다.** 2026-08-18 개편으로 원장이 길이
   *   축을 갖췄으니(조합마다 short·medium 한 행씩) 길이 토글 자체는 예문 경로에서도
   *   돌 수 있다. 그런데 이 자리는 길이와 `새로 받기` 를 **한 문**으로 묶어 두었고,
   *   `새로 받기` 쪽은 원장에 길이마다 한 행뿐이라 눌러도 같은 문장이 온다 — 죽은 버튼을
   *   세우지 않는다는 금지선에 걸린다. 예문 경로에 길이 토글만 세우려면 이 조건을 둘로
   *   가르는 판단이 먼저 있어야 한다(그 판단 전에는 지금 상태가 정직한 쪽이다).
   *
              ...
              <section className={styles.sect} aria-labelledby="msg-h">
                ...
                >
                    ...
                    */}
                    ...
                    {writing ? (
                      ...
                      /*
                        ...
                        사람의 것이다. 상한은 **360자**로 생성 상한(보통 340)보다 넓다 —
                        우리가 내준 문장을 그대로 두고도 손볼 자리가 남아야 한다.
                  ...
                  {/*
                    ...
                    길이와 새로 받기는 **한 문을 쓴다**(`canRegenerate` — 머리말에 근거).
                    본배포의 예문 톤에서는 그 문이 닫혀 있다: 원장은 길이마다 한 행뿐이라
                    `새로 받기` 를 눌러도 같은 문장이 오고, 눌러도 아무 일 없는 버튼은 갈 곳
                    없는 링크와 같은 거짓말이기 때문이다(바로 아래 사러 가기 섹션의 그 원칙).
                  */}
