...
/**
 * §1.5d — **적어 준 꽃이 3안에 못 들었을 때의 정직 한 줄** (2026-08-18).
 *
 * 규칙이 촘촘한 자리(고백 × 연인)에서는 에피소드에 꽃 이름을 직접 적어도 I·R 가점이
 * 그 이름을 이긴다(실측). 그때 점수를 비트는 대신 **화면이 이유를 말한다.**
 *
 * 세 갈래 중 이 필드가 서는 것은 하나뿐이다:
 *   · 언급한 꽃이 3안에 있다 → 아무것도 붙지 않는다(필드 없음).
 *   · 언급한 꽃이 제외됐다(반려동물·예산·향) → 붙지 않는다. 그 사정은 제외 쪽의 말이고,
 *     "더 가까운 셋을 골랐다" 는 여기서 할 말이 아니다(같은 자리에 두 이유를 세우지 않는다).
 *   · 3안 밖이고 제외도 아니다 → **여기가 이 필드의 자리다.**
 *
 * ⚠ **에피소드 원문을 되비추지 않는다.** 담는 것은 꽃 이름과 도감 id 뿐이다(§1.5j).
 */
export interface MentionedFlowerNote {
  /** 이름을 세울 꽃들. `id` 는 도감 상세(`/flowers/[slug]`)의 slug 와 같은 값이다. */
  flowers: { id: string; nameKo: string }[];
  /** 이름 앞에 오는 말. */
  lead: string;
  /** 이름 뒤에 이어지는 말. */
  tail: string;
...
export interface ResultPayload {
  ...
  ownWords: string[];
  /**
   * §1.5j — 맥락 칩 중 **우리가 적어 주신 이야기에서 읽어 낸 안전 신호**(반려동물·향).
   *
   * `ownWords` 와 같은 문법이다: 문장은 `contextChips` 안에 이미 있고, 이 목록은 화면이
   * 그 칸만 다른 결로 세우게 하는 표식이다. 사용자가 **고르지 않은** 신호로 후보가 줄었을
   * 때만 채워진다 — 읽기만 하고 아무 꽃도 빠지지 않았다면 비어 있다(일어나지 않은 일을
   * 알리지 않는다).
   * ⚠ 담기는 것은 **신호의 종류**뿐이다. 자유 서술 원문은 여기에도 들어가지 않는다.
   */
  readChips: string[];
  /** 이야기 목록의 결 필터 칩(전체 + 6종). 화면은 실제로 있는 결만 골라 세운다. */
  storyMoodFilters: StoryMoodFilter[];
  /**
   * 「이 결과 건네주기」 링크의 부호(`/r?c=…`). 만드는 규칙과 **싣지 않는 것**의 목록은
   * `share-link.ts` 머리말에 있다 — 자유 서술·멘트 본문은 여기 들어 있지 않다.
   */
  shareCode: string;
  /** §1.5d 적어 준 꽃 한 줄. 세 갈래 중 하나에서만 선다(`MentionedFlowerNote` 참고). */
  mentionedNote?: MentionedFlowerNote;
  /**
   * 예문 톤에서도 **길이·새로 받기를 세울 수 있는가** (2026-08-18).
   *
   * 기본은 없음(= 거짓)이다. 본배포의 예문 폴백에는 갈아 볼 다른 문장이 없어서
   * (`templates.csv` 는 조합마다 한 행뿐이다) 눌러도 아무 일이 없고, 그런 버튼은 갈 곳
   * 없는 링크와 같은 거짓말이라 세우지 않는다.
   *
   * **정적 데모만 이 값을 켠다.** 거기에는 손으로 쓴 변주 한 벌이 있어서
   * (`src/lib/demo/message-variants.ts`) 누르면 실제로 다른 문장이 온다 — 원칙이 금지한
   * 것은 "눌러도 아무 일 없는 버튼" 이지 버튼 자체가 아니다. 데모에서 동작하게 만든
   * 이상 숨길 이유가 없다(2026-08-18 사용자 확정).
   */
  canReword?: boolean;
}
...
/**
 * 공유 화면에 서는 꽃 한 칸 — **읽기 전용**이다.
 *
 * 결과 화면의 `FlowOptionView` 와 일부러 다른 타입이다. 그쪽은 색 고르기·이야기 시트·
 * 멘트·구매까지 딸린 조작 가능한 값이고, 이쪽은 "무엇을 골랐는지 보여 주는" 것이 전부다.
 * 한 타입으로 합치면 공유 화면이 쓰지도 않는 값을 실어 나르게 되고, 그 값 중에는
 * 링크에 실리지 않기로 한 것들이 섞여 들 자리가 생긴다.
 */
export interface SharedFlowerView {
  /** 도감 상세(`/flowers/[slug]`)로 건너가는 slug 이자 카탈로그 id. */
  flowerId: string;
  nameKo: string;
  scientificName: string;
  photo?: FlowerPhotoView;
  /** 그 꽃에 가장 널리 전해지는 한 줄. 못 찾으면 없다(지어내지 않는다). */
  meaningKo?: string;
  confidenceLabel?: string;
  /** 대표 이야기의 훅 한 줄, 훅이 없으면 제목. 이야기가 없으면 없다. */
  storyLine?: string;
}

/** 공유 화면이 통째로 받는 값. */
export interface SharePayload {
  /** `연인에게` · `고백` (+ 날짜) — 링크에 실린 어휘를 그대로 옮긴 칩. */
  chips: string[];
  flowers: SharedFlowerView[];
}

/** 공유 부호를 풀어 본 결과. 조작된 부호도 예외가 아니라 문장으로 돌아온다. */
...
/**
 * 흘러나오는 중인 멘트 — **표시용 조각**이다.
 *
 * ⚠ 이 값은 절대로 `ToneView.body` 가 되지 않는다. 복사·고쳐 쓰기·새로 받기 어느 것도
 *   이 글자를 만지지 못하고, 확정은 스트림이 끝난 뒤 계약(zod)을 통과한 톤 목록으로만
 *   일어난다. "스트리밍은 표시용, 확정은 검증 후" 라는 경계가 이 타입의 존재 이유다.
 */
export interface MessageStreamState {
  /** 아직 쓰고 있는가. 거짓이면 아래 조각은 비어 있고 화면은 확정된 톤만 세운다. */
  pending: boolean;
  /**
   * 톤 어휘 → 지금까지 흘러온 글자. **순서를 가정하지 않는다** — 프로바이더가 톤을 어떤
   * 차례로 쓰든 이 표는 이름으로 짝을 짓는다(폴백으로 다른 모델이 다시 쓰기 시작해도 같다).
   */
  drafts: Record<string, { headline?: string; body?: string }>;
}
