 ...
 *
 * ── 이 시트는 **도감에 없는 날에만** 열린다 (2026-08-18 확인) ─────────
 * 목록 줄이 그 갈림을 이미 지었다: 이어지는 날(366일 중 86일)은 `<Link>` 로 도감 상세에
 * 곧장 가고, 나머지만 `<button>` 으로 이 시트를 연다(`BirthDictionary` 의 `DictRow` —
 * 낭독기에서 "링크"와 "버튼"이 갈려 들려야 어디로 가는지 안다). 그래서 여기에 "도감에서
 * 더 보기" 다리를 달아 봐야 **뜰 자리가 없다**(크로스 링크 1차에서 넣었다가 걷어냈다).
 * 배선을 바꿀 생각이라면 아래 티어 고지도 함께 조건이 필요해진다 — design-spec §1.5o.
 *
 * ── 사진과 라이선스 ─────────────────────────────────────────────────
