...
export interface BirthFlowerView {
   ...
   * 없으면 화면은 "도감에는 아직 없는 꽃이에요" 쪽으로 간다(309일이 그렇다).
   */
...
export interface BirthDictEntry {
  ...
  alsoOn?: string;
  /**
   * 도감에 그 꽃이 있을 때만(366일 중 57일). 없는 것이 이 표의 정상 값이다.
   * `thumbSrc` 는 대표 실사 한 장(640px) — **없는 꽃은 사진을 지어내지 않는다.**
   */
  link?: { href: string; nameKo: string; thumbSrc?: string };
  /** 표 출처. 시트에서만 링크로 선다. */
  sourceUrl: string;
