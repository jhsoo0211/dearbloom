...
/**
 * 모바일 내비 메뉴 (§1.6c · 2026-08-16 사용자 지적).
 *
 ...
 *
 * 고쳐 놓는 방식은 "숨긴 것을 도로 펴기"가 아니다. 좁은 화면에서 그 줄을 접은 이유
 * (유리 알약 하나에 항목 다섯이 들어가지 않는다)는 그대로 유효하므로, **접힌 것을 여는
 * 문**을 내비 오른쪽에 세우고(§1.6b 아이콘 버튼 규격) 열면 화면 전체가 목차가 된다.
 *
 * ── 설계 메모 ────────────────────────────────────────────────────────
 * · 시트는 `<nav class="db-nav">` **밖**에 그린다. 내비는 `backdrop-filter` 를 쓰는
 ...
 * · 항목은 데스크톱 내비 넷 + `여러 명에게`(`/groups`) 다섯이다. `/groups` 를 내비에서
 *   내린 것은 #20 이었지만 그 결정은 **첫 화면에 진입점이 둘로 보이는 것**을 막으려던
 *   것이지 페이지를 숨기려던 것이 아니다. 이 시트는 첫 화면이 아니라 **펼쳐 본 전체 지도**
 *   라서, 여기에 있는 것은 §1.4c v3.4·#20 과 어긋나지 않는다.
 */
 ...
 * `/stories` 는 그쪽도 평범한 `<a>` 이고, 도감·편지·여러 명에게는 `next/link`(`prefetch` 없음)다.
 */
const SHEET_ITEMS = [
  ...
  { no: '05', label: '여러 명에게', href: '/groups', kind: 'link' },
] as const;
