...
export interface LandingData {
  ...
  basis: TodayBasis;
  /**
   * "오늘은 이 꽃을 꺼냈어요" — 리드 문단 전체 (§1.5n · 2026-08-16 개정).
   *
   * 꽃 이름 · 고른 이유 · **그 꽃 이야기 한 줄 인용**까지 한 문단으로 서버가 짓는다.
   * 이야기가 없으면 꽃말로, 꽃말도 없으면 사연 없이 물러선다. 모델 호출은 없다 —
   * 문장 틀 자체가 **날짜 씨앗으로 회전**할 뿐이라 같은 날 새로고침은 같은 문단이다.
   *
   * ⚠ 화면에서 이 문자열을 자르거나 앞에 날짜·꽃 이름을 덧붙이지 마라. 개정 전에는 화면이
