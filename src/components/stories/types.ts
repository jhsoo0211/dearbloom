...
export interface ArchiveLane {
  ...
  plate?: PlateView;
  /**
   * 이 꽃의 **대표 실사 한 컷** — 레인 안 카드의 미리보기 액자에 걸린다(2026-08-18 B-2).
   *
   * ── 왜 주소 한 줄인가 (`plate` 와 다른 점) ──────────────────────────
   * 도판은 액자가 각주(`sourceLine`)까지 쓰므로 `PlateView` 한 벌이 필요하지만, 카드의
   * 실사 액자가 쓰는 것은 **주소 하나**다(장식이라 alt 는 빈 문자열이고, 출처는 푸터의
   * 일괄 크레딧이 말한다). 그래서 뷰 타입을 새로 만들지 않고 좁힌 문자열 한 칸만 둔다 —
   * `@/lib/photos` 의 표(47종 × 주소·크레딧·설명)를 클라이언트 번들에 들이지 않는 경계는
   * `plate` 와 똑같다(코드 리뷰 P1-7). ⚠ 클라이언트에서 `photoFor()` 를 부르지 마라.
   *
   * 값은 `photoSrc(photo, 640)` 의 결과다 — 640 은 `PHOTO_SOURCES` 표가 허용하는 **가장
   * 좁은 폭**이고, 도감 카드·달력 칩·사전 썸네일이 이미 쓰는 값이라 사본이 재사용된다
   * (임의 폭을 새로 만들면 CDN 캐시만 갈라진다 — `photoSrcSet()` 머리말).
   *
   * 컷이 없는 꽃은 이 칸이 없고, 그때 카드는 **레인 헤더와 같은 도판**(`plate`)을 액자에
   * 건다 — 빈 상자를 세우지 않는다(브리프: "사진 없는 꽃은 도판 유지").
   */
  shotSrc?: string;
