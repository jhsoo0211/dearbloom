 ...
 *
 * ═══ 옆자리 파일 — `reads-links.ts` ═══════════════════════════════════
 * 원장의 `links_to` 를 **꽃 쪽에서 되짚는** 순수 조회(`readsForFlower`)는 이 파일이
 * 아니라 `./reads-links.ts` 에 산다. 결이 같아 여기 두고 싶어지겠지만, 그 함수를 부르는
 * 곳이 `app/recommend/build-result.ts` — 정적 데모에서 **브라우저 번들에 들어가는** 파일이라
 * 이 모듈을 import 하는 순간 아래 `node:fs`·`node:path` 가 클라이언트 그래프에 딸려 들어가
 * `npm run build:static` 이 죽는다. 옮기기 전에 그 파일의 머리말을 먼저 읽어라.
 */
