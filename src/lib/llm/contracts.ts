 ...
 * [2026-08-18 개정] 예문 표가 **두 벌을 갖췄다** — (intent × tone) 조합마다 short·medium
 * 이 각각 한 행씩 있다(62행). 그래서 이 축은 이제 생성 경로만의 것이 아니고
 * `buildTones` 가 함께 본다(`app/recommend/build-result.ts`). 예문 폴백도, 정적 데모도
 * 토글을 누르면 실제로 다른 분량의 문장을 내놓는다.
 */
...
/**
 * 길이 축이 뜻하는 **문장 수** — 모델에게 분량을 말할 때 쓰는 낱말.
 *
 * 자수보다 이쪽이 먼저다. 2026-08-18 사용자 신고("보통이 적어도 네다섯 문장은 되어야
 * 한다")의 실체는 자수가 아니라 **문장 수**였다 — 세 문장짜리 카드는 몇 자를 채워도
 * 짧게 읽힌다. 그래서 요구를 자수로만 번역하지 않고 문장 수로 그대로 적어 두고,
 * 자수 띠는 그 문장이 들어갈 그릇으로 쓴다(`prompt.ts` 의 `MESSAGE_LENGTH_CHARS`).
 *
 * ⚠ **문장 수를 세는 검증은 두지 않는다.** 한국어 문장 경계는 기계로 못 가른다
 *   (`…했어요, 그래서…` 한 줄이 두 문장일 수도 한 문장일 수도 있다). 이 값은 프롬프트가
 *   요구하는 **약속**이고, 강제는 자수 상한 하나가 맡는다.
 * ⚠ 화면(길이 칸)은 이 낱말을 쓰지 않고 자수 상한을 말한다. 화면이 말하는 수는 우리가
 *   **막을 수 있는 수**여야 하는데, 문장 수는 세지 않기로 한 값이라 막을 수가 없다.
 *   예문 원장에도 두세 문장을 넘는 짧은 벌이 있어(짧은 문장 넷이 95자) 화면이 문장 수를
 *   말하면 그 자리에서 곧바로 어긋난다.
 */
export const MESSAGE_LENGTH_SENTENCES: Record<MessageLength, string> = {
  short: '두세 문장',
  medium: '네다섯 문장',
};

/**
 * 멘트 한 편의 **글자 수 상한**(공백 포함).
 *
 * ── 이 상수가 계약 쪽에 사는 이유 ────────────────────────────────────
 ...
 *
 * ── 2026-08-18 개정: 한 칸씩 올렸다 ─────────────────────────────────
 * 처음 값(짧게 60 · 보통 120)은 "너무 길지 않게" 라는 요구를 관측치로 굳힌 것이었는데,
 * 같은 사용자가 반대편 신고를 했다: "지금 보통 길이가 짧게여야 하고, 보통은 적어도
 * 네다섯 문장은 되어야 한다." 그래서 **띠를 새로 만들지 않고 한 칸씩 밀었다** —
 * 어제의 `보통`(120)이 오늘의 `짧게`(140)이고, `보통`은 네다섯 문장이 들어갈 340 이다.
 *
 * ⚠ 이 수는 **계약이 막는 선**이지 우리가 요구하는 분량이 아니다. 요구하는 띠는
 *   `prompt.ts` 의 `MESSAGE_LENGTH_CHARS` 에 있고 그쪽이 더 좁다(보통 220~320).
 *   요구선과 차단선 사이의 여유가 곧 "네 문장 잘 써 놓고 스무 자 넘겨 버려지는 답" 을
 *   없앤다. 반대 방향(요구 > 차단)은 절대 안 된다 — 그러면 매 호출이 폴백이 된다.
 ...
 * ⚠ 결과 화면 **고쳐 쓰기**의 360자는 이 값과 별개다(`prompt.ts` 의 `MESSAGE_MAX_CHARS`).
