 ...
 *    순서는 gemini → claude → nvidia 이고, 있는 것만 줄을 선다.
 *  - **SDK 를 새로 설치하지 않는다.** 세 프로바이더 모두 REST 한 번이면 끝나고,
...
type ProviderName = 'gemini' | 'claude' | 'nvidia';
 ...
 *  2. claude — 품질 기준선.
 *  3. nvidia — 무료 크레딧. 앞의 둘이 쿼터를 소진했을 때 받는 자리다.
 *
 * `LLM_MODEL` 은 gemini·claude 의 모델명만 덮는다. NVIDIA 는 모델 이름 규칙이 아예
 * 달라서(`벤더/모델`) 같은 값을 나눠 쓸 수 없고, `NVIDIA_MODEL` 만 본다.
 */
 ...
 *
 * JSON 강제를 `response_format` 이 아니라 **프롬프트로** 한다. NIM 은 뒤에 붙는 모델마다
 * 이 필드의 지원 여부가 갈려서, 지원하지 않는 모델에 주면 400 으로 통째로 실패한다.
