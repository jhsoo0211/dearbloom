...
/**
 * 정적 데모의 **예문 변주** — 길이 토글과 `새로 받기` 가 실제로 돌게 하는 문안.
 *
 * 데모는 "안 깨지는 화면"이 아니라 **전 기능이 도는 화면**이어야 한다(2026-08-18 사용자
 * 확정). 그러려면 예문이 두 벌 이상 있어야 하는데, 원장(`templates.csv`)은 조합마다
 * 한 행뿐이라 손으로 쓴 변주가 데모 모듈에 따로 산다.
 *
 * 여기서 지키는 것
 *   ① **원장의 전 조합을 덮는다** — 어떤 마음·톤으로 들어와도 토글이 도는가.
 *   ② **화면이 말하는 분량 띠 안이다** — 60자라고 해 놓고 80자를 보여 주지 않는가.
  ...
  it('띠는 생성 경로가 지키는 그 띠다 — 데모와 본배포가 다른 분량을 보이지 않게', () => {
    for (const tones of Object.values(DEMO_MESSAGE_VARIANTS)) {
      for (const variant of Object.values(tones)) {
        for (const text of variant.short) {
          expect(text.length).toBeGreaterThanOrEqual(MESSAGE_LENGTH_CHARS.short.min);
          expect(text.length).toBeLessThanOrEqual(MESSAGE_LENGTH_CHARS.short.max);
        }
...
describe('데모 예문 변주 — 회전', () => {
  it('짧게는 누를 때마다 두 벌을 오간다', () => {
    const first = demoVariantBody('gratitude', 'plain', 'short', 1);
    const second = demoVariantBody('gratitude', 'plain', 'short', 2);
    expect(first).toBeDefined();
    expect(second).toBeDefined();
    expect(first).not.toBe(second);
    // 결정적이다 — 같은 차례면 같은 문장이다.
    expect(demoVariantBody('gratitude', 'plain', 'short', 3)).toBe(first);
  });

  it('보통은 원장 ↔ 손으로 쓴 한 벌을 오간다', () => {
    // 짝수 번째는 `undefined` = "지금 문장(원장)을 그대로 둔다".
    expect(demoVariantBody('gratitude', 'plain', 'medium', 0)).toBeUndefined();
    expect(demoVariantBody('gratitude', 'plain', 'medium', 2)).toBeUndefined();
    const alt = demoVariantBody('gratitude', 'plain', 'medium', 1);
    expect(alt).toBeDefined();
    expect(demoVariantBody('gratitude', 'plain', 'medium', 3)).toBe(alt);
  });
...
describe('데모 예문 변주 — 문안', () => {
  it('원장 문장을 그대로 베끼지 않았다', async () => {
    ...
    for (const tones of Object.values(DEMO_MESSAGE_VARIANTS)) {
      for (const variant of Object.values(tones)) {
        for (const text of [...variant.short, variant.mediumAlt]) {
  ...
  it('짧은 벌은 긴 예문의 앞부분을 자른 것이 아니다', async () => {
    ...
    for (const [intent, tones] of Object.entries(DEMO_MESSAGE_VARIANTS)) {
      for (const [tone, variant] of Object.entries(tones)) {
        const ledger = catalog.templates.find(
          (row) => row.intent === intent && row.tone === tone,
        )?.templateText;
        if (!ledger) continue;
        for (const text of variant.short) {
          expect(ledger.startsWith(text), `${intent}/${tone}`).toBe(false);
        }
      }
    }
  });
